...
## 0.3.2 — Production hardening candidate

This patch candidate supersedes the untagged 0.3.1 candidate. It completes
behavior already exposed by the 0.3 runtime and action contracts: bounded
hotkey, bar, action-bus, control, tray, and X11 paths; supervised shell
execution; generation-safe live bar controls; cancellation-safe diagnostics;
durable generated i3 files and rollback; lower idle rendering overhead; and an
early release-space preflight. It does not consume or redefine the planned
0.4.0 provider milestone.
