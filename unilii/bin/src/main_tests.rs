...
fn test_application_initialization_structure() {
    ...
    let config = Rc::new(RefCell::new(Some(unilii_core::config::Config::default())));
...
fn test_config_cloning_for_multiple_uses() {
    ...
    let config = unilii_core::config::Config::default();
...
fn test_window_settings_from_config() {
    ...
    let mut config = unilii_core::config::Config::default();
...
async fn test_module_loading_with_subscriptions() {
    ...
    configs.insert("clock".to_string(), ModuleConfig {
        enabled: true,
        position: unilii_core::ModulePosition::Left,
...
fn test_multi_panel_config_structure() {
    ...
    let config = unilii_core::config::Config {
        panels: vec![
